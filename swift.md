...
```swift
//예제
class ShopWebLoginQueue: Sequence, IteratorProtocol {
    // IteratorProtocol -> next()
    // Sequence -> makeIterator()
    fileprivate var loginInfos: [LoginShopInfo]
    fileprivate var index: Int
    
    init(_ loginInfos: [LoginShopInfo]) {
        self.loginInfos = loginInfos
        self.index = self.loginInfos.startIndex
    }
    
    func next() -> LoginShopInfo? {
        guard self.index < self.loginInfos.endIndex else {
            return nil
        }
        
        let info = self.loginInfos[self.index]
        self.index = self.loginInfos.index(after: self.index)
